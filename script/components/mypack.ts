...
function validateTopLevelNames(mcx: MyPackContext): boolean {
        ...
        ts.forEachChild(sourceFile, node => {
            if (ts.isVariableStatement(node)) {
                if (node.declarationList.declarations.length > 1) {
                    ...
                    logError('mypack', `${position} not support multiple declarations in variable declaration, I will not do that, when will that happen?`);
                    return;
                }
                ...
                logError('mypack', `${sourceFile.fileName}:${line + 1}:${character + 1}: not support dedicated export statement for now`); //, node);
                ...
                hasError = true;
                const { line, character } = sourceFile.getLineAndCharacterOfPosition(node.pos);
                logError('mypack', `${sourceFile.fileName}:${line + 1}:${character + 1}: unknown top level node kind: ${ts.SyntaxKind[node.kind]}`); //, node);
            }
        });
    ...
    for (const [fileName, names] of Object.entries(allNames)) {
        for (const name of names) {
            ...
            if (previousFileName) {
                hasError = true;
                logError('mypack', `${fileName} top level name ${name} has appeared in previous file ${previousFileName}`);
            }
        }
    }
        ...
        fileContent.split('\n').map((r, i) => [r, i + 1] as const).forEach(([line, rowNumber]) => {
            ...
            if (line.startsWith('*')) {
                ...
                if (!line.startsWith('as')) {
                    hasError = true;
                    logError('mypack', `${fileName}:${rowNumber}: ${raw}: invalid syntax, when will this happen? (1)`);
                    return;
                }
            ...
            if (line.startsWith('{')) {
                ...
                if (!line.startsWith('}')) {
                    hasError = true;
                    logError('mypack', `${fileName}:${rowNumber}: ${raw}: invalid syntax, when will this happen? (4)`);
                    return;
                }
            ...
            if (!line.startsWith('from ')) {
                hasError = true;
                logError('mypack', `${fileName}:${rowNumber}: ${raw}: invalid syntax, when will this happen? (5)`);
                return;
            }
            ...
            if (request.moduleName.startsWith('.')) {
                ...
                if (name) {
                    hasError = true;
                    logError('mypack', `${fileName}:${rowNumber}: ${raw}: not allow import alias in relative import for now`);
                    return;
                }
            }
...
function validateModuleDependencies(mcx: MyPackContext): boolean {
        ...
        for (const moduleImport of module.requests.filter(d => !d.moduleName.startsWith('.'))) {
            ...
            if (moduleImport.defaultName) {
                if (mergedImport.defaultName && mergedImport.defaultName != moduleImport.defaultName) {
                    hasError = true;
                    logError('mypack', `${module.path}: inconsistent default import from '${moduleImport.moduleName}', previous use ${mergedImport.defaultName}, here use ${moduleImport.defaultName}`);
                    ...
                    hasError = true;
                    logError('mypack', `${module.path}: default import ${moduleImport.defaultName} from '${moduleImport.moduleName}' has appeared in other import declarations from other modules`);
                } else if (mergedImport.namedNames.some(n => n.alias == moduleImport.defaultName)) {
                    hasError = true;
                    logError('mypack', `${module.path}: default import ${moduleImport.defaultName} from '${moduleImport.moduleName}' has appeared previous named imports from this module, when will this happen?`);
            ...
            if (moduleImport.namespaceName) {
                if (mergedImport.namespaceName && mergedImport.namespaceName != moduleImport.namespaceName) {
                    hasError = true;
                    logError('mypack', `${module.path}: inconsistent namespace import from '${moduleImport.moduleName}', previous use ${mergedImport.namespaceName}, here use ${moduleImport.namespaceName}`);
                } else if (mcx.externalRequests.some(o => o.moduleName != moduleImport.moduleName
                    ...
                    hasError = true;
                    logError('mypack', `${module.path}: namespace import ${moduleImport.namespaceName} from '${moduleImport.moduleName}' has appeared in other import declarations from other modules`);
                } else if (mergedImport.namedNames.some(n => n.alias == moduleImport.namespaceName)) {
                    hasError = true;
                    logError('mypack', `${module.path}: namespace import ${moduleImport.namespaceName} from '${moduleImport.moduleName}' has appeared previous named imports from this module, when will this happen?`);
            ...
            for (const namedName of moduleImport.namedNames) {
                if (mcx.externalRequests.some(o => o.moduleName != moduleImport.moduleName
                    ...
                    hasError = true;
                    logError('mypack', `${module.path}: import ${namedName.alias} from '${moduleImport.moduleName}' has appeared in other import declarations`);
                } else if (mergedImport.namespaceName == namedName.alias || mergedImport.defaultName == namedName.alias) {
                    hasError = true;
                    logError('mypack', `${module.path}: import ${namedName.alias} from '${moduleImport.moduleName}' has appeared previous namespace import or default import from this module, when will this happen?`);
                    ...
                    logError('mypack', `${module.path}: inconsistant import ${namedName.name} as ${namedName.alias} from '${moduleImport.moduleName}, previous is ${previous.name} as ${previous.alias}'`);
                }
    ...
    for (const module of mcx.modules) {
        for (const request of module.requests.filter(d => d.moduleName.startsWith('.'))) {
            ...
            if (!resolvedModuleName) {
                hasError = true;
                logError('mypack', `${module.path}: import '${request.moduleName}' not found, when will this happen?`);
                continue;
            }
    ...
    while (true) {
        ...
        if (depth >= 10) {
            hasError = true;
            logError('mypack', `too deep dependency or recursive dependency`, remainingRelationships);
            break;
        }
    }
...
async function cdnfy(mcx: MyPackContext): Promise<boolean> {
    ...
    try {
        ...
        logError('mypack', 'failed to read package.json in cdnfy', error);
        return false;
    }
    ...
    for (const request of mcx.externalRequests) {
        ...
        if (packages.length == 0) {
            hasError = true;
            logError('mypack', `external reference ${request.moduleName} not found package in package.json`);
            continue;
        }
