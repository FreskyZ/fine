...
try to install docker from release binaries instead of using package managers, because

- the package configuration for the docker engine (docker-ce) start the service immediately, this mean
  docker engine will at least start once with the default config, which may leave some unexpected system
  level effects from unwanted default config values, e.g. the following iptables rule issue
- not sure whether apt upgrade will stop docker for update, that will become accident
  for formal environments, although this poject is not that formal, I'd like to avoid this

docker have prebuilt binaries at https://docs.docker.com/engine/install/binaries/, but actually
you can inspect package files at https://download.docker.com/linux/debian/dists/trixie/pool/stable/amd64/
