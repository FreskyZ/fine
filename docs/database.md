...
- initdb --locale-provider=icu --locale=en_US.utf8
  ...
  - TODO use --no-instructions to avoid the pg_ctl start help message in automation setup
...
- \d, list many kind of things, use without parameter for tables, views, etc. which should be most common usage
...
- default logging destination is stderr, it is not convenient to read docker logs, and is lost when container is removed,
  so set log_destinations = 'stderr, csvlog' and logging_collector = on,
  TODO where is current_logfiles file
...
compare to mysql and mssql according to my years experience
TODO
