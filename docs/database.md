...
TODO really implement a import pg solution to save csv files and compare with pg_dump/psql /i

currently the backup plan is using pg_dump and TODO restore part
see docker-compose.yml and setup/backup-schedule.ts
...
- default primary key name is `{tablename}_pkey`
  default foreign key name is `{foreign table}_{reference table}_{reference key}_fkey`
