...
button.onclick = async () => {
    ...
    const response = await fetch(`https://api.domain.com/login`, { method: 'POST', headers: { 'X-Name': username, 'X-Access-Token': password } });
...
(async () => {
    if (localStorage['access-token']) {
        const response = await fetch(`https://api.domain.com/user-credential`, { headers: { 'X-Access-Token': localStorage['access-token'] } });
